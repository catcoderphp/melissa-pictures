var actions={};$(document).ready(function(){actions.main();});actions.main=function(){console.log('hola');$('#login-form').hide();actions.login();};actions.login=function(){$('#login-start').on('click',function(){$('#login-form').show('fade');$('#login-start').hide('fade');});};(function($){skel.breakpoints({xlarge:'(max-width: 1680px)',large:'(max-width: 1280px)',medium:'(max-width: 980px)',small:'(max-width: 736px)'});$(function(){var $window=$(window),$body=$('body');$body.addClass('is-loading');$window.on('load',function(){$body.removeClass('is-loading');});$('form').placeholder();skel.on('+medium -medium',function(){$.prioritize('.important\\28 medium\\29',skel.breakpoint('medium').active);});$('#nav > ul').dropotron({mode:'fade',noOpenerFade:true,speed:300});$('<div id="navToggle">'+'<a href="#navPanel" class="toggle"></a>'+'</div>').appendTo($body);$('<div id="navPanel">'+'<nav>'+
...
$('#navToggle, #navPanel, #page-wrapper').css('transition','none');});})(jQuery);(function($){$.fn.navList=function(){var $this=$(this);$a=$this.find('a'),b=[];$a.each(function(){var $this=$(this),indent=Math.max(0,$this.parents('li').length-1),href=$this.attr('href'),target=$this.attr('target');b.push('<a '+'class="link depth-'+indent+'"'+
